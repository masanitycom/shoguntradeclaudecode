...
SELECT 
    ...
    COUNT(CASE WHEN distribution_rate > 0 THEN 1 END) as with_distribution_rate
FROM mlm_ranks;
...
SELECT 
    ...
    AVG(COALESCE(total_earned, 0) / NULLIF(purchase_price, 0) * 100) as avg_roi_percent,
...
SELECT 
    ...
    ROUND(COALESCE(un.total_earned, 0) / NULLIF(un.purchase_price, 0) * 100, 2) as roi_percent,
    CASE 
...
-- 10. MLMランク設定確認
SELECT '⚙️ MLMランク設定確認' as section;
...
UNION ALL
SELECT 
    'MLMランク定義' as check_type,
    CASE 
        WHEN EXISTS (
            SELECT 1 FROM mlm_ranks 
            WHERE rank_level > 0 
            AND distribution_rate > 0
        ) THEN '✅ MLMランク定義完了'
        ELSE '❌ MLMランク定義不完全'
    END as status;

-- 12. 日利計算機能テスト
SELECT '🧪 日利計算機能テスト' as section;
...
-- 13. 最終ステータス
SELECT 
    '✅ システム全体テスト完了（完全版）' as final_status,
