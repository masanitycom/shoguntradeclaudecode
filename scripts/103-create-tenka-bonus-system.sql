...
BEGIN
  ...
  -- ランク保有者の分配率合計を計算
  WITH rank_totals AS (
    SELECT SUM(mr.bonus_percentage) as total_percentage
  ...
  FOR user_record IN
    ...
    CROSS JOIN rank_totals rt
    WHERE urh.is_current = true AND urh.rank_level > 0
  LOOP
    -- 個別ボーナス額を計算（分配率に応じて按分）
    bonus_amount := bonus_pool * (user_record.bonus_percentage / user_record.total_percentage);
...
-- 7. テスト用データ挿入
INSERT INTO weekly_profits (week_start_date, week_end_date, total_profit, bonus_rate)
VALUES 
  ('2024-01-01', '2024-01-07', 100000, 20.0),
  ('2024-01-08', '2024-01-14', 150000, 20.0)
