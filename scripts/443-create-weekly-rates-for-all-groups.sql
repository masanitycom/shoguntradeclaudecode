...
BEGIN
    ...
    -- 今週の日付を計算
    week_start_date := DATE_TRUNC('week', CURRENT_DATE) + INTERVAL '1 day'; -- 月曜日
    week_end_date := week_start_date + INTERVAL '6 days'; -- 日曜日
    week_number := EXTRACT(week FROM CURRENT_DATE)::INTEGER;
    
    debug_msg := '📅 対象週: ' || week_start_date || ' ～ ' || week_end_date || ' (第' || week_number || '週)';
    RAISE NOTICE '%', debug_msg;
