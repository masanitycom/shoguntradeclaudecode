...
SELECT 
    ...
    ra.task_answers as タスク回答,
    ra.fee_rate as 手数料率,
    ra.fee_amount as 手数料額,
    ra.net_amount as 純受取額,
    ra.status as ステータス,
    ra.applied_at as 申請日,
...
WHERE u.user_id IN ('pigret10', 'momochan', 'klmiklmi0204', 'katsuji1')
   OR u.email IN ('c781opigret@gmail.com', 'momokoshimizu0406@gmail.com', 'kimiko.0204.1357@gmail.com', 'jzs01473@gmail.com')
   OR u.name IN ('クリハラチアキ', 'コシミズモモ', 'アラホリキミコ', 'ムロツキカツジ')
ORDER BY u.user_id, ra.applied_at DESC;
...
SELECT 
    ...
    COUNT(ra.id) as レコード数,
    MAX(ra.applied_at) as 最新日時
FROM users u
LEFT JOIN reward_applications ra ON u.id = ra.user_id
WHERE u.user_id IN ('pigret10', 'momochan', 'klmiklmi0204', 'katsuji1')
   OR u.email IN ('c781opigret@gmail.com', 'momokoshimizu0406@gmail.com', 'kimiko.0204.1357@gmail.com', 'jzs01473@gmail.com')
...
SELECT 
    ...
    END as 進捗率,
    COUNT(dr.id) as 日利回数,
    SUM(CASE WHEN dr.is_claimed = true THEN dr.reward_amount ELSE 0 END) as 申請済み額,
    SUM(CASE WHEN dr.is_claimed = false THEN dr.reward_amount ELSE 0 END) as 未申請額,
    MIN(dr.reward_date) as 最初の報酬日,
    MAX(dr.reward_date) as 最後の報酬日
FROM users u
...
WHERE u.user_id IN ('pigret10', 'momochan', 'klmiklmi0204', 'katsuji1')
   OR u.email IN ('c781opigret@gmail.com', 'momokoshimizu0406@gmail.com', 'kimiko.0204.1357@gmail.com', 'jzs01473@gmail.com')
   OR u.name IN ('クリハラチアキ', 'コシミズモモ', 'アラホリキミコ', 'ムロツキカツジ')
GROUP BY u.id, u.user_id, u.name, n.name, un.current_investment, un.total_earned, un.max_earning
ORDER BY u.user_id, n.name;
...
WHERE u.user_id IN ('pigret10', 'momochan', 'klmiklmi0204', 'katsuji1')
   OR u.email IN ('c781opigret@gmail.com', 'momokoshimizu0406@gmail.com', 'kimiko.0204.1357@gmail.com', 'jzs01473@gmail.com')
   OR u.name IN ('クリハラチアキ', 'コシミズモモ', 'アラホリキミコ', 'ムロツキカツジ')
GROUP BY u.id, u.user_id, u.name, u.email, n.name, un.current_investment, un.total_earned, un.max_earning, un.is_active, u.created_at
ORDER BY u.user_id, n.name;
