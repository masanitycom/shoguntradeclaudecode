-- usersテーブルのID更新を最優先で実行

BEGIN;

-- Step 1: 一時IDのusersレコードのIDを正しいauth IDに直接更新
UPDATE users 
SET id = '3aa5c0de-8d2c-40b0-bdb5-3687d026ca5c'
WHERE id = '00000000-0000-0000-0000-000000000001';

-- Step 2: usersレコードの情報も正しく更新
UPDATE users 
SET 
    name = 'イシジマカツヒロ',
...
WHERE id = '3aa5c0de-8d2c-40b0-bdb5-3687d026ca5c'
  AND name = 'イシジマカツヒロ_temp';

COMMIT;

-- Step 3: 全ての関連データを正しいIDに更新（IDが存在するので安全）
BEGIN;
...
-- その他の可能性のあるテーブルも更新（存在しない場合はエラーにならない）
...
-- 一時データのクリーンアップ確認
SELECT 'Cleanup verification:' as cleanup;
SELECT COUNT(*) as remaining_temp_records 
FROM users 
WHERE email LIKE '%_temp%' OR user_id LIKE '%_TEMP%' OR name LIKE '%_temp%';