...
BEGIN
  ...
  -- 未申請報酬がある全ユーザーを処理（正しいカラム名を使用）
  FOR user_record IN
    SELECT 
      u.id,
      u.name,
      COALESCE(SUM(ra.amount), 0) as unclaimed_rewards
    FROM users u
    ...
    HAVING COALESCE(SUM(ra.amount), 0) > 0
  LOOP
...
-- テスト実行
SELECT 'Testing compound processing with correct columns...' as status;
