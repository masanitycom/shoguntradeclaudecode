-- 1. 既存関数を削除
DROP FUNCTION IF EXISTS test_new_user_registration();
...
-- 7. テスト用の新規ユーザー作成関数（修正版）
CREATE FUNCTION test_new_user_registration()
RETURNS TABLE(
    test_result TEXT,
    new_user_id UUID,
    initial_rank TEXT
...
BEGIN
    -- テスト用ユーザーID生成
    test_user_uuid := gen_random_uuid();
    ...
    -- テストユーザー削除
    DELETE FROM user_rank_history WHERE user_id = test_user_uuid;
    DELETE FROM users WHERE id = test_user_uuid;
END;
$$ LANGUAGE plpgsql;
