...
BEGIN
    ...
    IF day_of_week = 0 OR day_of_week = 6 THEN
        result := json_build_object(
            'success', false,
            'message', format('土日は計算対象外です: %s', target_date::TEXT),
            'processed_count', 0,
            'total_amount', 0
        );
        RETURN result;
    ...
    result := json_build_object(
        'success', true,
        'message', format('%s件の報酬を計算しました（合計: $%s）', processed_count, ROUND(total_amount, 2)),
...
-- 6. 権限設定
GRANT EXECUTE ON FUNCTION calculate_daily_rewards_emergency(DATE) TO authenticated;

-- 完了メッセージ
SELECT 'Emergency calculation system fixed with correct table structure' as status;
