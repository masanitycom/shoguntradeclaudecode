...
-- 5. ohtakiyoユーザーに足軽ランクを付与（テスト用）
INSERT INTO user_rank_history (user_id, rank_level, is_current, created_at)
SELECT 
  u.id,
  1, -- 足軽
  true,
  NOW()
FROM users u 
WHERE u.name = 'ohtakiyo'
ON CONFLICT (user_id) WHERE is_current = true 
DO UPDATE SET rank_level = 1, updated_at = NOW();
