-- テーブル構造に基づいた正しい過去分計算関数

-- user_nftsテーブルの実際の構造確認結果:
-- purchase_price (投資額)
-- current_investment (現在の投資額)
-- purchase_date (購入日)

-- 正しい過去分計算関数を作成
CREATE OR REPLACE FUNCTION calculate_nft_historical_rewards_correct(
    start_week INTEGER,
    end_week INTEGER
)
RETURNS TABLE(
        ...
        FOR week_num IN start_week..end_week LOOP
            ...
            FOR user_nft_record IN 
                SELECT 
                    ...
                    un.user_id,
                    un.current_investment as amount,  -- current_investmentを使用
                    un.purchase_date
                FROM user_nfts un
...
-- 関数実行前の確認
SELECT 'Function created successfully. Ready to execute historical calculation.' as status;
