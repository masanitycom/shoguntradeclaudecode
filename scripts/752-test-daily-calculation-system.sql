...
SELECT 
    '=== 日利計算実行 ===' as section,
    calculate_daily_rewards('2025-02-10'::date) as calculation_result;
...
SELECT 
    ...
    SUM(un.total_earned) as total_earned_before,
    (SUM(un.total_earned) + SUM(dr.reward_amount)) as total_earned_after
FROM daily_rewards dr
JOIN user_nfts un ON dr.user_nft_id = un.id
JOIN users u ON dr.user_id = u.id
WHERE dr.reward_date = '2025-02-10'
