    ...
    FOR i IN 0..27 LOOP -- 4週間 = 28日
        ...
        IF EXTRACT(dow FROM calc_date) BETWEEN 1 AND 5 THEN
            ...
            LOOP
                ...
                LOOP
                    -- 既存の報酬レコードをチェック（user_nft_idとreward_dateの組み合わせ）
                        ...
                        IF reward_amount > 0 THEN
                            ...
                            BEGIN
                                ...
                                IF total_processed % 50 = 0 THEN
                                    RAISE NOTICE '処理済み: %件 (ユーザー: %, NFT: %, 日利: %, 報酬: %)', 
                                        total_processed, user_record.user_name, nft_record.user_nft_id, daily_rate, reward_amount;
                                END IF;
                                
                            EXCEPTION WHEN OTHERS THEN
                                RAISE NOTICE 'エラー発生: ユーザー% NFT% 日付% - %', 
                                    user_record.user_id, nft_record.user_nft_id, calc_date, SQLERRM;
                                
                                -- エラーが発生した場合は、より多くのカラムを含むINSERTを試行
                                BEGIN
                                    INSERT INTO daily_rewards (
                                        user_id,
                                        user_nft_id,
                                    ...
                                    total_processed := total_processed + 1;
                                    RAISE NOTICE '再試行成功: %件目', total_processed;
                                    
                                EXCEPTION WHEN OTHERS THEN
                                    RAISE NOTICE '再試行も失敗: ユーザー% NFT% 日付% - %', 
                                        user_record.user_id, nft_record.user_nft_id, calc_date, SQLERRM;
                                END;
                            END;
                        END IF;
                    ELSE
                        -- 既存レコードがある場合はスキップ
                        -- RAISE NOTICE 'スキップ: ユーザー% NFT% 日付% (既存)', 
                        --     user_record.user_id, nft_record.user_nft_id, calc_date;
                    END IF;
                END LOOP;
            END LOOP;
        END IF;
