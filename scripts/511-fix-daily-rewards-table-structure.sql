-- daily_rewardsテーブル構造の修正

-- 1. 現在のテーブル構造を確認
SELECT 
  'Current daily_rewards structure' as info,
  column_name,
  data_type,
  is_nullable,
  column_default
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE daily_rewards ADD COLUMN purchase_amount DECIMAL(10,2);
    RAISE NOTICE 'purchase_amountカラムを追加しました';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE daily_rewards ADD COLUMN investment_amount DECIMAL(10,2);
    RAISE NOTICE 'investment_amountカラムを追加しました';
  END IF;
END $$;
...
-- 4. 修正されたテーブル構造を確認
SELECT 
  'Updated daily_rewards structure' as info,
  column_name,
  data_type,
  is_nullable
FROM information_schema.columns 
