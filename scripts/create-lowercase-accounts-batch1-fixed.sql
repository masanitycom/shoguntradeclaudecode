-- 大文字版対応ありの小文字アカウント作成（バッチ1修正版: 短いuser_id）

SELECT '=== CREATING LOWERCASE ACCOUNTS BATCH 1 FIXED ===' as section;
...
SELECT 
    ...
    name, 'b@shogun-trade.com' as email, user_id || '_lc' as user_id,
...
SELECT 
    ...
    name, 'c@shogun-trade.com' as email, user_id || '_lc' as user_id,
    phone, referrer_id, usdt_address, wallet_type, is_admin,
    user_id || '_lc' as my_referral_code,
    'https://shogun-trade.vercel.app/register?ref=' || user_id || '_lc' as referral_link,
...
SELECT 
    ...
    name, 'd@shogun-trade.com' as email, user_id || '_lc' as user_id,
    phone, referrer_id, usdt_address, wallet_type, is_admin,
    user_id || '_lc' as my_referral_code,
    'https://shogun-trade.vercel.app/register?ref=' || user_id || '_lc' as referral_link,
...
SELECT 
    ...
    name, 'e@shogun-trade.com' as email, user_id || '_lc' as user_id,
    phone, referrer_id, usdt_address, wallet_type, is_admin,
    user_id || '_lc' as my_referral_code,
    'https://shogun-trade.vercel.app/register?ref=' || user_id || '_lc' as referral_link,
...
SELECT 
    ...
    name, 'f@shogun-trade.com' as email, user_id || '_lc' as user_id,
    phone, referrer_id, usdt_address, wallet_type, is_admin,
    user_id || '_lc' as my_referral_code,
    'https://shogun-trade.vercel.app/register?ref=' || user_id || '_lc' as referral_link,
...
SELECT 'BATCH 1 FIXED COMPLETED - 5 lowercase accounts created!' as batch1_status;

-- 確認クエリ
SELECT 'Created accounts verification:' as verification;
SELECT id, name, email, user_id FROM users 
WHERE id IN (
    'b6647604-8baa-4158-be6c-10ed45ac7bc5',
    'd898dae2-57b6-48d2-ae12-27a0fa9d7c52',
    'fdbbfcd5-80a8-4c59-b9bf-68684d56bd03',
    '90ee9886-1149-4fd9-bd7c-7f3b611be5b1',
    'bc5acf81-0d47-4485-b456-3143ff493b24'
);

SELECT 'SUCCESS: First 5 lowercase accounts ready for login!' as success_message;