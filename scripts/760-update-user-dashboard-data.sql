...
AND dr.reward_amount >= 50  -- 最低申請額50ドル
GROUP BY u.id, u.name, u.total_earned
HAVING SUM(dr.reward_amount) >= 50
...
SELECT 
    ...
    SUM(un.purchase_price) as total_investment,
    COALESCE(AVG(dr.reward_amount), 0) as avg_daily_reward,
    COALESCE(SUM(dr.reward_amount), 0) as total_daily_rewards
FROM user_nfts un
...
SELECT 
    ...
    jsonb_build_object(
        ...
        'total_investment', (SELECT COALESCE(SUM(total_investment), 0) FROM users),
        'total_earned', (SELECT COALESCE(SUM(total_earned), 0) FROM users),
        'today_rewards', (
