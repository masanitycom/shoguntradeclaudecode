                ...
                LOOP
                    ...
                    IF existing_reward = 0 THEN
                        ...
                        IF reward_amount > 0 THEN
                            -- 全ての必須カラムを含むINSERT（week_start_dateも追加）
                            ...
                            IF total_processed % 50 = 0 THEN
                                RAISE NOTICE '処理済み: %件 (ユーザー: %, NFT: %, 週開始: %, 日利: %, 報酬: %)', 
                                    total_processed, user_record.user_name, nft_record.user_nft_id, week_start, daily_rate, reward_amount;
                            END IF;
                        END IF;
                    ELSE
                        -- 既存レコードがある場合はスキップ
