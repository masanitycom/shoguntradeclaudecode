...
ON CONFLICT (rank_level) DO UPDATE SET
  ...
  required_nft_value = EXCLUDED.required_nft_value,
  max_organization_volume = EXCLUDED.max_organization_volume,
  other_lines_volume = EXCLUDED.other_lines_volume,
...
-- 7. テスト実行
SELECT 'MLM rank system created successfully' as status;
