...
BEGIN
    ...
    BEGIN
        ...
        WITH calculation_data AS (
            ...
            AND COALESCE(un.total_earned, 0) < un.purchase_price * 3 -- 300%未満
        )
        ...
        SET is_active = false,
            updated_at = NOW()
        WHERE COALESCE(total_earned, 0) >= purchase_price * 3
        AND is_active = true;
...
SELECT '✅ 週利設定と日利計算の修正完了（実際のテーブル構造対応版）' as final_status;
